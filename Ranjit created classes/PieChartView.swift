    ...
    override func draw(_ rect: CGRect) {
        ...
        for segment in segments {
            ...
            if showSegmentLabels { // do text rendering
                ...
                var textToRender = "No Record"
                if segment.name == "No Record" {
                    textToRender = showSegmentValueInLabel ? "0%\n\(segment.name) " : segment.name
                }
                else{
                   textToRender = showSegmentValueInLabel ? "\(segment.value.formattedToOneDecimalPlace)%\n\(segment.name) " : segment.name
                }
